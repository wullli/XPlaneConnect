...
class XPlaneConnect(object):
    ...
    def sendCMD(self, cmd_string):
        """Executes the specific command once

            Args:
              cmd_string: The string of the command to execute
        """

        buffer = struct.pack(b"<4sx", b"CMND")

        # Preconditions
        if len(cmd_string) == 0 or len(cmd_string) > 255:
            raise ValueError("dref must be a non-empty string less than 256 characters.")
        else:
            fmt = "<B{0:d}sBf".format(len(cmd_string))
            buffer += struct.pack(fmt.encode(), len(cmd_string), cmd_string.encode(), 1, 0)
